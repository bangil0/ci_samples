...
class Renew_password extends Auth_Controller {
    ...
    public function index() {
        ...
        $this->quick_page_setup(Settings_model::$db_config['active_theme'], 'main', $this->lang->line('password_renew_title'), 'renew_password', 'header', 'footer');
    }
    ...
    public function send_password() {
        ...
        if (isset($data['active']) && $data['active'] != 1) {
            ...
            if ($this->renew_password_model->insert_recover_password_data($data['user_id'], $token, $this->input->post('email'))) {
                ...
                $this->email->subject($this->lang->line('password_forgot_subject'));
                $this->email->message($this->lang->line('email_greeting') ." ". $data['username'] . $this->lang->line('password_forgot_message') ."\r\n\r\n". base_url() ."auth/renew_password/reset/". urlencode($this->input->post('email')) ."/". $token);
                $this->email->send();
                $this->session->set_flashdata('success', '<p>'. $this->lang->line('password_forgot_success') .'</p>');
            }else{
    ...
    public function reset($email, $token) {
        ...
        if (isset($time_diff) && $time_diff > Settings_model::$db_config['password_link_expires']) {
            ...
            if ($this->renew_password_model->update_password_and_nonce(urldecode($email), $new_password)) {
                ...
                $this->email->message($this->lang->line('email_greeting') ." ". $data['username'] . $this->lang->line('password_reset_message'). addslashes($new_password));

                $this->email->send();
                $this->session->set_flashdata('success', '<p>'. $this->lang->line('password_reset_success') .'</p>');
                redirect("login");
            }else{
                $this->session->set_flashdata('error', '<p>'. $this->lang->line('password_reset_failed_db') .'</p>');
            }
        }else{
            $this->session->set_flashdata('error', '<p>'. $this->lang->line('password_reset_failed_token') .'</p>');
        }
        redirect("renew_password");
