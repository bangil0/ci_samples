...
class List_members extends Admin_Controller {
    ...
    public function index($order_by = "username", $sort_order = "asc", $search = "all", $offset = 0) {
        ...
        if ($search == "post") {
            ...
            $this->form_validation->set_rules('username', $this->lang->line('username'), 'trim|max_length[16]');
            $this->form_validation->set_rules('first_name', $this->lang->line('first_name'), 'trim|max_length[40]');
            $this->form_validation->set_rules('last_name', $this->lang->line('last_name'), 'trim|max_length[60]');
            $this->form_validation->set_rules('email', $this->lang->line('email_address'), 'trim|max_length[255]');
        ...
        // set layout data
        $this->quick_page_setup(Settings_model::$db_config['adminpanel_theme'], 'adminpanel', $this->lang->line('list_members'), 'list_members', 'header', 'footer', '', $content_data);
    }
    ...
    public function toggle_active($id, $username, $offset, $order_by, $sort_order, $search, $active) {
        ...
        if ($this->list_members_model->get_username_by_id($id) == ADMINISTRATOR) {
            $this->session->set_flashdata('error', '<p>'. $this->lang->line('list_members_admin_nodeactivate') .'.</p>');
            redirect('/adminpanel/list_members/index');
            return;
